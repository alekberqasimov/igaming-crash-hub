(()=>{'use strict';
const M={prefix:'camelrushv2lc',finance:'camelrushv2lc_finance_v2',logs:'camelrushv2lc_logs_v2',reset:'camelrushv2lc_reset_v2',camel:true};
...
function render(){if(IS_ADMIN)return;const b=box();if(!b)return;const h=list().slice(-18);b.innerHTML=h.map(x=>{const m=Number(x.multiplier),c=m<1.5?'low':m<5?'mid':m<10?'high':'legend';return '<span class="history-pill '+c+'" data-shared-round="'+x.round+'">'+m.toFixed(2)+'x</span>'}).join('');requestAnimationFrame(()=>{b.scrollLeft=b.scrollWidth})}
