(()=>{'use strict';
const VERSION='1.3.2',KEY='sonicrush_config_v2';
...
function afterSave(){setTimeout(()=>{toPercentUI();const c=read();window.dispatchEvent(new CustomEvent('igch:sonic:config-saved',{detail:{version:VERSION,targetProfitMargin:c.targetProfitMargin,targetRtp:c.targetRtp}}))},0)}
...
document.addEventListener('click',e=>{const save=e.target.closest?.('[data-save]');if(save){beforeSave();afterSave()}const defs=e.target.closest?.('[data-defaults]');if(defs)setTimeout(toPercentUI,0)},true);
setInterval(toPercentUI,350);toPercentUI();
window.IGCHSonicPercentConfig={version:VERSION,refresh:toPercentUI};
})();